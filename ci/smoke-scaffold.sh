...
echo "== Guard: check for prerelease resolutions in pubspec.yaml"
# A -dev/-beta caret means no all-stable set exists right now (analyzer-major
# migrations regularly cause this). It's a drift signal worth surfacing, but
# the real pass/fail is the codegen + analyze gate below — broken prereleases
# (e.g. freezed 3.0.0-0.0.dev generating nothing) fail there deterministically.
if grep -nE '\^?[0-9]+\.[0-9]+\.[0-9]+-(dev|beta|alpha)' pubspec.yaml; then
    echo "WARNING: prerelease dependency resolved (see above) — ecosystem is mid-migration; relying on the analyze gate below." >&2
fi
